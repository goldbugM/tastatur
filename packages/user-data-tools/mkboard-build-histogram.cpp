...
void build_histogram(const fs::path &path) {
  ...
  mkboard::stats_data stats_data{buffer};
...
int main(int argc, char *argv[]) {
  ...
  fs::path root{mkboard::DEFAULT_ROOT};
  ...
  mkboard::hist_speed.fill(0);
  mkboard::hist_acc.fill(0);

  for (fs::directory_entry const &entry :
       fs::recursive_directory_iterator(root)) {
    if (entry.is_regular_file()) {
      const fs::path &path = entry.path();
      mkboard::stats_file_id file_id;
      if (mkboard::parse_stats_file_name(path, file_id)) {
        if (file_id.user_id != 0 && file_id.backup_num == 0) {
          mkboard::build_histogram(path);
        }
      }
    }
  }

  mkboard::normalize_histogram(mkboard::hist_speed);
  mkboard::print_histogram(mkboard::hist_speed);

  mkboard::normalize_histogram(mkboard::hist_acc);
  mkboard::print_histogram(mkboard::hist_acc);
