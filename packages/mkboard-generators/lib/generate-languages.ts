...
import { TransitionTableBuilder } from "@mkboard/phonetic-model";
...
function generate(language: Language): void {
  ...
  const modelPath = pathTo(`../mkboard-phonetic-model/assets/model-${id}.data`);
  const wordsPath = pathTo(`../mkboard-content-words/lib/data/words-${id}.json`);
