...
import { User, UserLoginRequest } from "@mkboard/database";
...
test("send a new access token", async () => {
  ...
  const userLoginRequest = await UserLoginRequest.findByEmail("test@mkboard.com");
  isNotNull(userLoginRequest);

  const user = await User.findByEmail("test@mkboard.com");
  isNull(user);

  const [message] = context.mailer.dump();
  equal(message.to, "test@mkboard.com");
...
test("login with an access token / new user", async () => {
  ...
  equal(await request.who(), "test@mkboard.com");
