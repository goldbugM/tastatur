...
import { User, type UserExternalId } from "@mkboard/database";
import {
  AbstractAdapter,
  AccessToken,
  type ClientConfig,
...
test("redirect to provider", async () => {
  ...
  equal(
    ...
    "https://www.mkboard.com/auth/oauth-callback/fake",
  );
});
...
test("validate state", async () => {
  ...
  // Act. Step 1: redirect from mkboard to provider.
  ...
  isNull(await User.findByEmail("fake@mkboard.com"));
  isNull(await request.who());

  // Act. Step 2: redirect from provider to mkboard.
...
test("login a new user", async () => {
  ...
  isNotNull(await User.findByEmail("fake@mkboard.com"));
  equal(await request.who(), "fake@mkboard.com");
});
