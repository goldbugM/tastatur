...
test("generate unique user name", async (ctx) => {
  ...
  await User.query().insertGraph({
    email: `test@mkboard.com`,
    name: `test`,
    createdAt: now,
  });
  for (let i = 1; i <= 9; i++) {
    await User.query().insertGraph({
      email: `test${i}@mkboard.com`,
      name: `test${i}`,
      createdAt: now,
    });
  }
  await User.query().insertGraph({
    email: `example@mkboard.com`,
    name: `example`,
    createdAt: now,
  });
  ...
  equal(
    await User.findUniqueName(null, "x".repeat(100) + "@mkboard.com"),
    "x".repeat(32),
  );
  equal(await User.findUniqueName(null, "unique"), "unique");
  equal(await User.findUniqueName(null, "unique@mkboard.com"), "unique");
  equal(await User.findUniqueName(null, "test"), "test10");
  equal(await User.findUniqueName(null, "test@mkboard.com"), "test10");
  equal(await User.findUniqueName("test@mkboard.com", "test"), "test");
  equal(await User.findUniqueName("test@mkboard.com", "test@mkboard.com"), "test");
  equal(await User.findUniqueName(null, "test10"), "test10");
  equal(await User.findUniqueName(null, "test10@mkboard.com"), "test10");
  equal(await User.findUniqueName("test@mkboard.com", "example"), "example1");
  equal(
    await User.findUniqueName("test@mkboard.com", "example@mkboard.com"),
    "example1",
  );
});
...
test("create user from resource owner with null values", async (ctx) => {
  ...
  const email = "example1@mkboard.com";
...
test("create access token", async (ctx) => {
  ...
  equal(await UserLoginRequest.init("example1@mkboard.com"), "token1");
  isNull(await User.findByEmail("example1@mkboard.com"));
  deepEqual(
    (await UserLoginRequest.findByEmail("example1@mkboard.com"))!.toJSON(),
    {
      id: 1,
      email: "example1@mkboard.com",
      accessToken: "token1",
  ...
  equal(await UserLoginRequest.init("example1@mkboard.com"), "token1");
  equal(await User.findByEmail("example1@mkboard.com"), null);
  deepEqual(
    (await UserLoginRequest.findByEmail("example1@mkboard.com"))!.toJSON(),
    {
      id: 1,
...
test("delete expired access token", async (ctx) => {
  ...
  isNotNull(await UserLoginRequest.findByEmail("example1@mkboard.com"));
  ...
  isNull(await UserLoginRequest.findByEmail("example1@mkboard.com"));
...
test("login with a valid access token", async (ctx) => {
  ...
  isNotNull(await User.findByEmail("example1@mkboard.com"));
  isNotNull(await UserLoginRequest.findByEmail("example1@mkboard.com"));
...
test("convert to user details", async (ctx) => {
  ...
  deepEqual((await User.findByEmail("user1@mkboard.com"))?.toDetails(), {
    id: "55vdtk1",
